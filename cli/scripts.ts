...
export const configProject = async () => {
    ...
    const payerWallet = feePayerWalletKeypair.publicKey;
    ...
    // Try to fetch existing config, or use defaults for first time init
    let gameRound = new BN(0);
    try {
        const configAccount = await program.account.config.fetch(configPda);
        console.log("Existing config found:", configAccount);
